//Blue are white women -- some are Swifties, a bit hard to distinguish.  Orange are asian men -- the people who are Swifties are more distinguishable.  
...
display('single-profit0', formatNumberWithCommas(singleModel.profit));
//display('profit0', formatNumberWithCommas(comparisonExample0.profit));
//display('profit1', formatNumberWithCommas(comparisonExample1.profit));
display('total-profit', formatNumberWithCommas(comparisonExample0.profit + comparisonExample1.profit));
...
GroupModel.prototype.classify = function(threshold) {
  ...
  this.fpr = fpr(this.items);
  this.fnr = fnr(this.items);
...
// DL Calculate false positive rate (This is chat gpt code) -- DL adjusted to call this fnr
function fpr(items) {
  var totalBad = 0;
  var totalBadPredictedGood = 0;
  items.forEach(function(item) {
    if (item.value == 0) {
      totalBad += 1;
      totalBadPredictedGood += item.predicted;
    }
  });
  if (totalBad == 0) {
    return 1;
  }
  return totalBadPredictedGood / totalBad;
}

// DL Calculate false neg rate (This is chat gpt code)
function fnr(items) {
  var totalGood = 0;
  var totalGoodPredictedBad = 0;
  items.forEach(function(item) {
    if (item.value == 1) {
      totalGood += 1;
      totalGoodPredictedBad += (1 - item.predicted);
    }
  });
  if (totalGood == 0) {
    return 1;
  }
  return totalGoodPredictedBad / totalGood;
}
...
function createHistogram(id, model, noThreshold, includeAnnotation) {
  ...
  var thresholdLabel = svg.append('text').text('ticket threshold: 50')
      .attr('x', tx)
      .attr('y', 40)
  ...
  function setThreshold(t, eventFromUser) {
    ...
    thresholdLabel.attr('x', labelX).text('ticket threshold: ' + t);
    if (includeAnnotation) {
...
function createHistogramLegend(id, category) {
  ...
  var textPad = 4;
  svg.append('text')
      .text('no ticket / Swiftie')
      .attr('x', centerX - boxSide - textPad)
      .attr('y', 2 * boxSide - textPad)
      .attr('text-anchor', 'end')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('no ticket / non Swiftie')
      .attr('x', centerX - boxSide - textPad)
      .attr('y', boxSide - textPad)
      .attr('text-anchor', 'end')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('given ticket / Swiftie')
      .attr('x', centerX + boxSide + textPad)
      .attr('y', 2 * boxSide - textPad)
      .attr('text-anchor', 'start')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('given ticket / non Swiftie')
      .attr('x', centerX + boxSide + textPad)
      .attr('y', boxSide - textPad)
      .attr('text-anchor', 'start')
      .attr('class', 'legend-label');
...
//DL Create all 4 pie charts (TPR, TP, FPR, FNR) chat gpt code
function createRatePies(id, model, palette, includeAnnotations) {
    console.log("createRatePies function executed for ID:", id);
    var width = 600;
    var lx = 0;
    var height = 170;
    var svg = d3.select('#' + id).append('svg')
    ...
    var tprPie = createPie('tpr-' + id, [1,1], tprColors, svg, 45, cy, 40);
    var fprPie = createPie('fpr-' + id, [1,1], fprColors, svg, 145, cy, 40);
    var fnrPie = createPie('fnr-' + id, [1,1], fnrColors, svg, 245, cy, 40);
    var allPie = createPie('all-' + id, [1,1,1,1], palette, svg, 345, cy, 40);
    
    var topY = 35;
    
    var tprLabel = createPercentLabel(svg, lx, topY, 'TPR', 'pie-label', 'pie-number');
    var fprLabel = createPercentLabel(svg, lx + 100, topY, 'FPR', 'pie-label', 'pie-number');
    var fnrLabel = createPercentLabel(svg, lx + 200, topY, 'FNR', 'pie-label', 'pie-number');
    var posLabel = createPercentLabel(svg, lx + 300, topY, 'PR', 'pie-label', 'pie-number');

explanation(svg, ['% of Swifties', 'w/ tickets'], 0, topY);
explanation(svg, ['% of non-Swifties', 'w/ tickets'], 100, topY);
explanation(svg, ['% of Swifties', 'w/o tickets'], 200, topY);
explanation(svg, ['% of all', 'w/ tickets'], 300, topY);


model.addListener(function() {
    var items = model.items;
    
        // Logging the values to see if they're correct
    console.log("model.tpr:", model.tpr);
    console.log("model.fpr:", model.fpr);
    console.log("model.fnr:", model.fnr);
    
    tprPie([countMatches(items, 1, 1), countMatches(items, 1, 0)]);
    fprPie([countMatches(items, 0, 1), countMatches(items, 0, 0)]);
    fnrPie([countMatches(items, 1, 0), countMatches(items, 1, 1)]);
    allPie([countMatches(items, 1, 0), countMatches(items, 0, 0), countMatches(items, 1, 1), countMatches(items, 0, 1)]);
...
function createCorrectnessMatrix(id, model) {
  ...
  explanation(svg, ['Swifties given tickets',
      'plus non-Swifties', 'not given tickets'], 0, topY);
  explanation(svg, ['Swifties denyed tickets',
      'plus non-Swifties', 'given tickets'], width / 2 + 4, topY);
